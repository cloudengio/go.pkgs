...
func TestAnnotated(t *testing.T) {
	...
	if got, want := err.Error(), "errors/caller_test.go:31: open /a/b: file does not exist"; got != want {
...
func TestPrintf(t *testing.T) {
	...
	if got, want := fmt.Sprintf("%v", err), "errors/caller_test.go:58: open /a/b: file does not exist"; got != want {
		t.Errorf("got %v, want %v", got, want)
	}
	if got, want := fmt.Sprintf("%+v", err), "errors/caller_test.go:58: open /a/b: file does not exist"; got != want {
		t.Errorf("got %v, want %v", got, want)
	}
	if got, want := fmt.Sprintf("%#v", err), `errors/caller_test.go:58: &os.PathError{Op:"open", Path:"/a/b", Err:(*errors.errorString)`; !strings.HasPrefix(got, want) {
		t.Errorf("got %v, want %v", got, want)
	}
}
