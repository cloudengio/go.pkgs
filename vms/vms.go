...
var transitionTable = map[State]map[Action]State{
	StateInitial: {
		...
		// Allow deleting from the initial state to clean up after a clone that
		// failed, or was interrupted, part way through.
...
// it if necessary. Running VMs are stopped before deletion.
// It returns an error if the instance is not Stopped or Deleted after the cleanup
// or if the cleanup fails for any other reason.
//
...
// instances are deleted on a best-effort basis and any error from that delete
// is discarded since there is most likely nothing to delete.
func CleanupVM(ctx context.Context, inst Instance, timeout time.Duration) error {
	...
	case StateRunning:
		...
		if s != StateStopped {
			...
			errs.Append(runErr)
			errs.Append(stopErr)
			return fmt.Errorf("cleanup: expected VM to be stopped after stopping, in state %s, run/stop err: %w", s, errs.Err())
		}
		fallthrough
