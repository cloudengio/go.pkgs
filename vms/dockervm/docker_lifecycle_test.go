...
// Package dockervm_test contains lifecycle and integration tests.
// These tests require Docker to be installed and running and are configured
// as level 1 long-running tests as per cloudeng.io/cicd. They can be
// enabled by setting CLOUDENG_LONG_RUNNING_TESTS=1 or by referring to the
// test names directly.
...
func cleanupTestContainers(ctx context.Context) {
	...
	for _, name := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		...
		if info, found, err := dockervm.InspectContainer(ctx, name); err == nil && found {
