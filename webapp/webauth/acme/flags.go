...
// ManagerConfig represents the configuration required to create an autocert.Manager
...
// NewManager creates a new autocert.Manager from the supplied config.
...
func NewManager(ctx context.Context, cache autocert.Cache, cl ManagerConfig, allowedHosts ...string) (*autocert.Manager, error) {
	...
	if len(cl.ClientHost) == 0 {
		return nil, fmt.Errorf("must specify a value for client-host")
	}
	allowed := append([]string{cl.ClientHost}, allowedHosts...)
